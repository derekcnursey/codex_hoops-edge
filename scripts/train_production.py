#!/usr/bin/env python3
"""Train production model: a0.85_p10 adjusted, 53 features, seasons 2015-2025."""
...
from src.trainer import fit_scaler, save_checkpoint, train_classifier, train_regressor
...
print(f"\nLoading features...")
df = load_multi_season_features(SEASONS, no_garbage=True, adj_suffix=ADJ_SUFFIX)
...
reg_hp_full = {**reg_hp, "epochs": 100}
regressor = train_regressor(X_scaled, y_spread, hparams=reg_hp_full)
save_checkpoint(regressor, "regressor", hparams=reg_hp,
                feature_order=config.FEATURE_ORDER)

# Train classifier
print("\nTraining MLPClassifier (BCE)...")
cls_hp_full = {**cls_hp, "epochs": 100}
classifier = train_classifier(X_scaled, y_win, hparams=cls_hp_full)
